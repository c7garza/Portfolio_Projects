...
Skills Used: Queries with constraints, Aggregate functions, data type conversion, Joins, Window function, CTE, Creating Views
...
FROM Portfolio_Project..Covid_Deaths$
WHERE continent  IS NOT NULL -- IS NOT NULL to filter out only the continents
GROUP BY continent
ORDER BY Total_Death_Count DESC;
...
--Displays what percentage of population has received at least one vaccine; with a rolling count column of new vaccinations
...
---CTE
--Using CTE to perform calculation on Partition created in previous query.Creates a column that displays the percentage of population vaccinated using Cumulative_Vac_Count 
